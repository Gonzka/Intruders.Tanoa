...
while {_unit getVariable ["isCloaked", false] && alive _unit} do {
	// create emitters
	...
	waitUntil {player distance _unit > 20 || !(_unit getVariable ["isCloaked", false])}; // wait until player is too far away
	...
	while {player distance _unit > 20 && (_unit getVariable ["isCloaked", false])} do {
