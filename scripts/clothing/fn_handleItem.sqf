...
_details = [_item] call gonzka_fnc_fetchCfgDetails;
if (count _details isEqualTo 0) exitWith {};

if (_bool) then {
    switch (_details select 6) do {
	    case "CfgFaces": {
			[player, _item] remoteExec ["setFace", 0, player];
		};
        ...
        case "CfgVehicles": {
            ...
            if (!isNil "_items") then {
                {[_x,true,true,false,true] call gonzka_fnc_handleItem; } forEach _items;
            };
        };
                        ...
                        if (_ispack) then {
                            ...
                            if (_override) then{
                                ...
                                if (vest player isEqualTo _item && {!_preview}) then {
                                    ...
                                    if (!isNil "_items") then {
                                        {[_x,true,false,false,true] spawn gonzka_fnc_handleItem;} forEach _items;
                                    };
                                };
                            };
                        };
    ...
    switch (_details select 6) do {
		case "CfgFaces": {
			[player, profileFace] remoteExec ["setFace", 0, player];
		};
