...
if (_section isEqualTo "") then {
    _section = switch (true) do {
		...
		case (isClass(configFile >> "CfgFaces" >> "Man_A3" >> _className)): {"CfgFaces"};
    };
};

if (!(_section isEqualType "") || {_section isEqualTo ""}) exitWith {[]};
//if (!(_section isEqualType "") || {!isClass(configFile >> _section >> _className)} || {_section isEqualTo ""}) exitWith {[]};

if (_section isEqualTo "CfgFaces") then {
    _config = configFile >> _section >> "Man_A3" >> _className;
} else {
